...
export const useAddAvailability = ({
  ...
  const handleAddAvailability = useCallback(() => {
    ...
    if (vals.isAvailable) {
      ...
      if (!vals.modes || vals.modes.length === 0)
        return { success: false, message: 'Please select a service mode' };
      if (!vals.selectedTimeSlots || vals.selectedTimeSlots.length === 0)
        return { success: false, message: 'Please select time slots' };
    }
  ...
  const generateTimeSlots = (start: Date, end: Date, intervalMinutes: number) => {
    if (!start || !end)
      return { success: false, message: 'Please select the startTime and endTime' };
