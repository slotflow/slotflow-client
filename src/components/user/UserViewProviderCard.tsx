...
const UserViewProviderCard = ({ provider, serviceDetails }: UserViewProviderCardProps) => {
  ...
  const gradient = cardGradients[(provider?._id?.charCodeAt(0) || 0) % cardGradients.length];
    ...
    >
      <CardContent className="p-4 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-lg leading-tight">{provider?.username}</h3>
        ...
        <div className="space-y-1">
          <p className="text-sm font-medium">{serviceDetails?.serviceName}</p>
