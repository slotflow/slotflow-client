...
const containsActiveHeading = (heading: TOCHeadingProps, activeId: string): boolean => {
  if (heading.id === activeId) return true;

  return heading.children?.some((child) => containsActiveHeading(child, activeId)) ?? false;
};
...
const TOC = ({ headings }: TOCProps) => {
  ...
  const renderHeadings = (items: TOCHeadingProps[], level = 0) => {
    return items.map((heading) => {
      const expanded = containsActiveHeading(heading, activeId);
      ...
      return (
        <div key={heading.id}>
          ...
          {heading.children && heading.children.length > 0 && expanded && (
            <div className="mt-1 space-y-1 ml-2">{renderHeadings(heading.children, level + 1)}</div>
          )}
        </div>
      );
    });
  ...
  return (
    <aside className="hidden lg:block">
      <div className="sticky top-24">
        ...
        <nav className="space-y-2 cursor-pointer">{renderHeadings(headings)}</nav>
      </div>
    </aside>
  );
};
