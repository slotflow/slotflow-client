    ...
    <div className="w-full flex items-center gap-6 p-6 rounded-xl border bg-muted/30">
      <div className="relative group">
        <img
          ...
          alt="Profile"
          className={`h-32 w-32 rounded-xl object-cover border shadow-sm transition ${
            profileImageUpdating ? 'opacity-50' : ''
          }`}
        />
      ...
      <div className="flex flex-col justify-center">
        <h1 className="text-2xl md:text-3xl font-bold">{authUser?.username}</h1>
        <p className="text-sm text-muted-foreground">Click on image to update profile picture</p>
      </div>
    </div>
  );
};
