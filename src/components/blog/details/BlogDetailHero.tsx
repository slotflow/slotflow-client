    ...
    <section id="hero" className="relative h-[70vh] min-h-[600px] overflow-hidden w-full">
      ...
      <div className="relative mx-auto flex h-full items-end px-6 pb-16 max-w-7xl mx-auto">
        <motion.div
          ...
          className="max-w-4xl"
        >
          <Badge className="rounded-full px-4 py-1">{category}</Badge>
          <h1
            title={title}
            className="line-clamp-2 mt-6 text-5xl font-black leading-tight lg:text-7xl"
          >
            {title}
          </h1>
          <p
            title={description}
            className="line-clamp-2 mt-8 max-w-3xl text-lg leading-8 dark:text-white/60 text-black/60"
          >
            {description}
          </p>
          <div className="mt-10 flex flex-wrap items-center gap-8">
            <div className="flex items-center gap-3">
              <Avatar className="h-12 w-12">
                ...
                <AvatarFallback>{author?.author.slice(0, 2)}</AvatarFallback>
              </Avatar>
              <div>
                <h4 className="font-semibold">{author?.author}</h4>
                <p className="text-sm dark:text-white/70 text-black/70">{author?.proffession}</p>
              </div>
            </div>
