...
const FAQAccordion = ({ faqs, loading = false }: FAQAccordionProps) => {
  if (loading) {
    ...
    return (
      <section id="no-faqs" className="w-full">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-center py-24 text-center">
          ...
          <h2 className="mt-8 text-3xl font-bold tracking-tight">No FAQs Found</h2>

          <p className="mt-4 max-w-lg text-base leading-7 text-muted-foreground">
            We couldn't find any FAQs matching your search. Try another keyword or browse our Help
            Center for more answers.
          </p>
        </div>
      </section>
    );
  ...
  return (
    <section id="according">
      <Accordion type="multiple" className="mx-auto mt-20 flex max-w-5xl flex-col gap-6">
        {faqs.map((item) => (
