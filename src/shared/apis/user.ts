...
export const userFetchMyProfileDetails = async (): Promise<
  ApiBaseResponse<UserFetchMyProfileDetailsResponse>
> => {
...
export const userUpdateInfo = createAsyncThunk<
  ApiBaseResponse<UserUpdateUserInfoResponse>,
  UserUpdateUserInfoRequest
>('/user/updaterUserInfo', async (data: UserUpdateUserInfoRequest) => {
...
export const fetchUserProfileDetails = async (
  userId: string,
): Promise<ApiBaseResponse<AdminFetchUserProfileDetailsResponse>> => {
  const response = await axiosInstance.get(`/users/${userId}`);
  return response.data;
};

// provider fetching users for chat
export const fetchUsersForChat = async (): Promise<
  ApiBaseResponse<FetchUsersForChatSidebarResponse>
> => {
